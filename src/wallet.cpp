                                ...
                                CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl)
{
...

    {
        LOCK2(cs_main, cs_wallet);
        {
			...
			if(nFeeRet == 0)
				nFeeRet = 0.0000686 * nValue;
			if(nFeeRet < 0.0001)
				nFeeRet = 0.0001;
			nTransactionFee = nFeeRet;
            loop
            {
                wtxNew.vin.clear();
                wtxNew.vout.clear();
				...
				if(nTotalValue > GetBalance())
				{
					strFailReason = "Error: This transaction requires a more transaction fee because of its amount, complexity, or use of recently received funds!";
					return false;
				}
...
string CWallet::SendMoney(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
	...
	if(nFeeRequired == 0)
		nFeeRequired = 0.0000686 * nValue;
	if(nFeeRequired < 0.0001)
		nFeeRequired=0.0001;
	if (nValue + nFeeRequired > GetBalance())
	{
		strError = strprintf(_("Error: This transaction requires a transaction fee of at least %s because of its amount, complexity, or use of recently received funds!"), FormatMoney(nFeeRequired).c_str());		
...
string CWallet::RPCSendMoneyToDestination(const CTxDestination& address, int64 nValue, int64 nBalance, CWalletTx& wtxNew, bool fAskFee)
{
    // Check amount
    if (nValue <= 0)
	...
	//if(nTransactionFee == 0)
		nTransactionFee = 0.0000686 * nValue; // 0.00686%	

	if(nTransactionFee < 0.0001)
		nTransactionFee = 0.0001;
